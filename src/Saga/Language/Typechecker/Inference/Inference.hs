...
class
  ...
  , Generalize (Classifier e)
    ...
    infer       :: (t ~ Classifier e, w ~ Constraint t, InferM w m)  => e -> m e
    lookup      :: (t ~ Classifier e, w ~ Constraint t, InferM w m)  => String -> m (Qualified t)
    fresh       :: (t ~ Classifier e, w ~ Constraint t, InferM w m)  => Tag a -> m (VarType e a)

    --qualify     :: (t ~ Classifier e, w ~ Constraint t)              => w -> Polymorphic t -> Polymorphic t

    emit        :: (t ~ Classifier e, w ~ Constraint t, InferM w m) => w -> m ()
    inform      :: (t ~ Classifier e, w ~ Constraint t, InferM w m) => Info -> m ()
    emit = emit'
    inform = inform'


class Instantiate t where
    instantiate :: (w ~ Constraint t, InferM w m) => Polymorphic t -> t -> m (Polymorphic t)

class Generalize t where
    generalize ::  (w ~ Constraint t, InferM w m) => t -> m (Polymorphic t)
