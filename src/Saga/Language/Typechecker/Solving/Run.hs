...
process :: Solving es => Solver.Constraint -> Eff es [Solver.Constraint]
process constraint = process' (Shared.flatten constraint) []
    where
...
instance Entails Constraint where
    ...
    entails c@(Solver.Implementation {}) cs = Protocols.entails c cs
    entails c@(Solver.Refinement {}) cs     = Refinements.entails c cs
...
instance Solve Solver.Constraint where
    ...
    solve c@(Solver.Refinement {})     = Refinements.solve c

    -- | TODO:QUESTION #35 How can this be moved to its own file? It's only here to prevent cyclic import dependencies.
